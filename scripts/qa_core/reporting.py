#!/usr/bin/env python3
"""Business-neutral HTML/Markdown report presentation."""
...
def write_html_report(
    *,
    ...
    gallery_html = f'<section class="evidence-block"><h2>{html.escape(str(evidence.get("images_title", "页面证据")))} <small>点击图片查看原图</small></h2><p class="section-note">{html.escape(str(evidence.get("image_note", "截图展示页面状态；业务结论由调用方的结构化断言提供。")))}</p><div class="gallery">{images}</div></section>' if images else ""
    if images and evidence.get("images_first"):
        evidence_html += gallery_html
    if checks:
        evidence_html += f'<section class="evidence-block"><h2>{html.escape(str(evidence.get("checks_title", "断言核对")))} <small>{html.escape(str(evidence.get("checks_note", str(len([item for item in check_items if isinstance(item, dict)])) + " 条断言")))}</small></h2><div class="table-wrap"><table><thead><tr>{check_headers}</tr></thead><tbody>{checks}</tbody></table></div></section>'
    ...
    ui_style = ".table-wrap td:first-child{white-space:nowrap}.gallery{grid-template-columns:repeat(auto-fit,minmax(220px,1fr))}" if layout == "ui" else ""
    if layout == "gallery":
    ...
    generated_at = generated_at or format_east8_time()
...
{ui_style}</style></head><body><main><header><div><h1>{html.escape(title)}</h1><div class="meta">{meta_html} · 生成时间：{html.escape(generated_at)}</div></div><div class="verdict {status_class(verdict)}"><strong>{html.escape(verdict)}</strong><span>{html.escape(verdict_detail)}</span></div></header><div class="metrics">{cards}</div><nav class="flow-grid">{navigation}</nav>{evidence_html}{''.join(sections)}</main></body></html>"""
