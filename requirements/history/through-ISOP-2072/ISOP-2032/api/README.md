...
```bash
python3 scripts/run-requirement-api.py ISOP-2032
python3 scripts/run-requirement-api.py ISOP-2032 --env .env.fat --execute --insecure
```
...
历史结论见 [2026-09-10 FAT 执行记录](../../../../../docs/history/requirement-records-20260907-11.md#isop-2032-20260910)。定向复测可增加 `--only <组合ID> ...`，每次产生独立证据。

本需求[总用例](../cases.csv)与[API数据驱动用例](data-cases.csv)分开；后者由本目录cases.json生成，保留请求、断言、总用例关联及前置缺口。更新命令：`npm run qa:cases -- ISOP-2032`，不登录。
