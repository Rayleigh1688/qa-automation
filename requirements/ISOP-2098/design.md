...
- 范围：仅百家乐投注详情；不因链接锚点为二十一点而扩大实现范围。
- 主来源：[Jira ISOP-2098](https://alibaba-international.atlassian.net/browse/ISOP-2098)。
...
| --- | --- | --- | --- | --- |
| AC-01 | 待补预期 | 百家乐详情字段映射 | J正文“只需要写百家乐”；Q-01 | ISOP-2098-C01 |
| AC-02 | 待补预期 | 其他游戏回归边界 | J正文；Q-01 | ISOP-2098-C02 |

具体步骤和断言维护在[test-cases.md](test-cases.md)，问题答复维护在[questions.md](questions.md)，避免重复维护公式。

## 影响与回归选择

仅百家乐投注详情；不因链接锚点为二十一点而扩大实现范围。 身份、时间、金额或类型映射必须由独立样本佐证；旧结果不能代替本次验证。查询与计算优先API和只读对账；页面、布局、真实客户端行为由人工UI提供证据。
...
| --- | --- | --- | --- |
| CAP-01 / ISOP-2098-C01 | 百家乐详情字段映射；样本与断言见用例 | 未映射；不猜URL/字段 | Q-01 |
| CAP-02 / ISOP-2098-C02 | 其他游戏回归边界；样本与断言见用例 | 未映射；不猜URL/字段 | Q-01 |
