...
- 范围：GCash回跳及状态展示；目前只有“跳转后流程/bug”提纲，尚无可判定的具体修复预期。
- 主来源：[Jira ISOP-2093](https://alibaba-international.atlassian.net/browse/ISOP-2093)。
...
| --- | --- | --- | --- | --- |
| AC-01 | 待补预期 | 成功回跳状态一致性 | J正文及视频待核；Q-01、Q-02 | ISOP-2093-C01 |
| AC-02 | 待补预期 | 取消/失败/未支付回跳 | J正文及视频待核；Q-01、Q-02 | ISOP-2093-C02 |

具体步骤和断言维护在[test-cases.md](test-cases.md)，问题答复维护在[questions.md](questions.md)，避免重复维护公式。

## 影响与回归选择

GCash回跳及状态展示；目前只有“跳转后流程/bug”提纲，尚无可判定的具体修复预期。 身份、时间、金额或类型映射必须由独立样本佐证；旧结果不能代替本次验证。查询与计算优先API和只读对账；页面、布局、真实客户端行为由人工UI提供证据。
...
| --- | --- | --- | --- |
| CAP-01 / ISOP-2093-C01 | 成功回跳状态一致性；样本与断言见用例 | 未映射；不猜URL/字段 | Q-01、Q-02 |
| CAP-02 / ISOP-2093-C02 | 取消/失败/未支付回跳；样本与断言见用例 | 未映射；不猜URL/字段 | Q-01、Q-02 |
