...
| --- | --- | --- | --- | --- |
| AC-01 | 待补预期 | 活动页二期数据（预备） | J提纲；2104关联待确认；Q-01 | ISOP-2118-C01 |
...
正文仅活动页数据与SQL调整；与2104 A版的关系及是否影响计返仍待明确。 身份、时间、金额或类型映射必须由独立样本佐证；旧结果不能代替本次验证。查询与计算优先API和只读对账；页面、布局、真实客户端行为由人工UI提供证据。
...
| --- | --- | --- | --- |
| CAP-01 / ISOP-2118-C01 | 活动页二期数据（预备）；样本与断言见用例 | 未映射；不猜URL/字段 | Q-01 |
| CAP-02 / ISOP-2118-C02 | SQL改动前后对账（预备）；样本与断言见用例 | 未映射；不猜URL/字段 | Q-01、Q-02 |
