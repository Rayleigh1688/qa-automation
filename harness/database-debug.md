...
以下仅是当时账号的诊断快照，不代表当前账号前置或当前 P0 门禁：

- `fb_members.kyc_status = 5`，说明 KYC 已通过。
- `fb_members.state = 0`，会员状态正常。
- 2026-08-31 执行固定单注 1000 的 6 次真实 UI 投注前，共有 5 条未完成流水、合计 5750。
- 6 次投注后总剩余流水为 0，钱包接口返回 `locked=0`、`withdrawable=884376.46`。
- 流水清零后 Maya 可创建 1000 提现单；后台审核因 FAT 未配置可用转账接口而取消，金额自动退回。

因此，当时提现 P0 的剩余阻塞已从“流水前置”收敛为“FAT 无可用转账接口”，不是会员数据或余额问题。
