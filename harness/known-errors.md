...
- 每个 API runner 和每次 UI 命令都 fresh login，不跨命令复用 `p0-api-session.json` 或 storage state；同一 runner/Playwright suite 内可以串行共享本轮认证状态。
- UAT UI 只由 global setup 触发一次本轮动态短信登录；不要在单个 suite 内为每条用例重复发码。若 fresh login 本身受限，必须更换专用测试账号或解除号码限制，不能回退复用历史 token。
...
- 历史处理（2026-09-04 移出范围前）：该反例保持失败，不能自动勾选条款或放宽断言。当前状态：缺陷未修复、默认套件已排除该 Case。

处理：

- 2026-09-02 用户明确接受本轮临时例外后继续 UAT 受控资金链；2026-09-04 FAT fresh-login 旧默认套件连续三轮稳定复现为 10/11。用户随后决定将该项从当前默认 UI 自动化用例和固定测试点中移除；缺陷事实仍保留在本记录中，但之后的默认 UI 报告不再包含或统计此 Case，不能把范围移除表述为产品缺陷已修复。
...
- 历史资金测试账号（具体标识仅保留在本机配置） 使用真实 UI 固定单注 1000 完成 6 注后，全部未完成流水由 5750 降为 0；钱包 `locked=0`、`withdrawable=884376.46`。
...
## UI-FUND-20260908：免费局导致点击数与付费注单数不同

- 环境/状态：FAT，2026-09-08 实测，当前受控 UI 资金链停止条件。
- 最小复现：Lucky Penny 单注100，执行18次Spin后进入/经历免费局；客户端查询29条已结算记录仅5条投注额100，其余24条为0。净输赢与钱包变化一致，基础流水只减少500。
- 影响：按剩余流水计算下一批需要累计31次点击，超过20上限；不得将18次点击表述为18笔付费投注或流水完成。当前停止且不进入提现，未自动清流。
- 处理：报告分开记录点击、付费注单、零投注额记录及实际流水；执行器达到上限先保存中间结果再失败退出。后续预算/免费局计数策略待完善，不通过放宽PASS断言或API提现规避。
- 证据：`ui/results/client-game-bet-smoke.json`、`api/results/ui-bet-stop-diagnosis.json`；当前账号状态与后续动作仅维护在 [交接](../AI-HANDOFF.md)。
