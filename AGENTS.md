...
- 完成后说明改动、实际验证及未验证边界，并同步当前交接；保留的日期结论标明来源；用户明确授权清理历史时同步标注原始证据已退出，不冒充当前实测。

- 新需求设计从 [requirements/README.md](requirements/README.md) 进入；旧扫描已删除，[退出索引](archive/interface-scans/README.md)仅保留导航；当前接口资产从api/inventory、api/catalog、api/p0读取，不恢复旧专项脚本作为门禁。
