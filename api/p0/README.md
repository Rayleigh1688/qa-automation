...
- `account_lane` 是执行约束：safe smoke 客户端查询使用 `mature_read_account`；资金正向链路从充值到提现统一使用 `fund_flow_account`；KYC 和独立流水限制反例使用各自 lane。
...
6. 提现金额必须同时满足余额和通道限制。提现创建后，后台只处理并核对本轮产生的订单；任一阶段业务失败时停止，不调用后续成功接口制造通过。
7. 数据库只用于只读诊断和交叉核对，不直接修改 KYC、余额、流水、充值或提现状态。

## 推荐命令

执行当前 P0 默认套件（包含受控充值写入，资金链推进到充值与补单检查点）：
...
当前资产为 8 条主流程、60 条整体 P0 用例，其中 31 条 safe smoke（客户端 18、后台 13）、15 条登记反例（默认执行 13 条，充值上下限 2 条为显式缺陷探针），其余为受控写、UI 协作或测试数据阻塞项。更换成熟账号后 FAT safe smoke 已通过 31/31，默认安全反例通过 13/13。

1. 新号注册，随后进入 KYC；现有成熟账号只承担稳定只读检查和 KYC 后业务。
2. KYC 未通过、待审或已驳回时可复用同一账号提交；一旦通过，重复提交必须作为反例拒绝。
3. 充值、投注和提现复用同一个 `fund_flow_account`；主链路只做不参加活动的充值。普通存款基础流水在提现前完成，但不专门发起流水限制反例。
4. 投注之后先核对投注/派彩记录，再核对钱包与前后台账变，最后进入提现。
5. 活动配置、盲盒、Filcoin、VIP、代理、收藏等接口已移出 P0 门禁；活动流水限制保留为独立专项，暂不阻塞主正向链路。
...
当前核对深度：客户端账变与后台账变的结构查询已分别进入 safe smoke；充值补单和提现审核已能按本次订单定位。订单号、uid、金额、方向和最终状态的一对一自动关联断言仍是 P0 收尾项，不能把“两个列表都能查”表述为已完成数据核对。
...
- `RESTRICTED_CLIENT_PHONE`：只补充活动流水专项；暂不参与主流程放行判断。
