...
当前完整 P0 以本轮 Maya 提现单创建、进入 `under_review`、前后台订单号/uid/金额一致作为提交阶段接受标准。最终出款成功仍是目标闭环，但当前环境例外不将它作为本阶段必选通过项。

历史取消/退款核对是独立证据，不要求每轮先取消才能通过。若显式运行审核阶段且业务失败，仍须停止并记录失败，不能用此接受标准把失败的审核请求改判成功，也不能继续调用 success。

## 充值列表时间窗口

2026-09-08 FAT 同一 UI 充值订单只读对照：`/admin/finance/deposit/risk/list` 与 `/admin/finance/deposit/list` 的秒窗口均未找到本轮订单，毫秒窗口精确匹配 UID/金额。共享查询与模板已修正为毫秒，不带时间的查询会被业务拒绝。此结果不推广到其他列表接口；UAT 尚未随该修正联网重验。诊断证据与当前链路结果见 [交接](../../AI-HANDOFF.md)。

2026-09-08 后续 UI 全流程对账对照证实 `/admin/finance/transaction/list` 同样使用毫秒：相同 UID/窗口的秒查询为空，毫秒查询返回本轮已知账变。公共契约和 P0 模板同步修正；仅 FAT 已验证。客户端账变行 `uid` 当前返回空字符串，使用已验证钱包会话确定客户端账号，再与后台真实 UID 及账变 ID/金额逐条核对。
